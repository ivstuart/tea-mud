...
public class ClanPromoteTest {
    ...
    public void testClanPromote() {
        ...
        ClanPromote clanPromote = new ClanPromote();
        clanPromote.execute(mob, player1Mob.getName());
        clanPromote.execute(mob, player1Mob.getName());

        assertEquals("Player1 level 3", 3, player1Mob.getPlayer().getClanMembership().getLevel());

        ClanDemote clanDemote = new ClanDemote();
        clanDemote.execute(mob, player1Mob.getName());

        assertEquals("Player1 level 2", 2, player1Mob.getPlayer().getClanMembership().getLevel());
