...
public class ClanCreateTest {
    ...
    @Test
    public void testClanCreate() {
        Race human = new Race();
        World.getInstance().addToWorld(human);

        Mob mob = TestHelper.makeDefaultPlayerMob("ted");

        SomeMoney cash = new Money(Money.COPPER, 10000);
        mob.getInventory().add(cash);

        ClanCreate clanCreate = new ClanCreate();
        clanCreate.execute(mob, "Newbie clan");


        assertNotNull("Clan created", Clans.getClan(0));
    }
